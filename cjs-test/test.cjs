...
it('should work with postcss-nested (issue #19)', () =>
    postcss([
        ...
        .then(result =>
            assert.strictEqual(result.css, '.touch .c:hover{color:red}')
        )
);
...
try {
    describe('should pass csso compress tests', () => {
        ...
        const require$1 = module$1.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('test.cjs', document.baseURI).href)));

        const compressTests = require$1('../node_modules/csso/cjs-test/fixture/compress.cjs');
